...
def mean_loss(model: Any, tokenized: list[list[int]]) -> tuple[float, np.ndarray]:
...
def evaluate_update_candidates(
    ...
    """Evaluate both sign conventions for diagnosis.

    In normal gradient descent, if release_sign estimates sign(<grad, z>), the
    update is theta - lr * sign * z.  We also evaluate theta + lr * sign * z to
    make smoke-test diagnostics explicit when the chosen random direction is noisy.
    """

    minus_value = original - lr * float(release_sign) * direction
    plus_value = original + lr * float(release_sign) * direction
    minus_losses = losses_for_param_value(model, tokenized, param_path, minus_value)
    plus_losses = losses_for_param_value(model, tokenized, param_path, plus_value)
    return {
        "descent_loss_mean": float(minus_losses.mean()),
        "opposite_loss_mean": float(plus_losses.mean()),
        "descent_losses": minus_losses,
        "opposite_losses": plus_losses,
    }
...
def main() -> int:
    ...
    parser.add_argument("--json-out", type=Path, default=Path("benchmark-results/paczero_mlxlm_real_param_update_smoke_results.json"))
    args = parser.parse_args()

    start = time.perf_counter()
    print("# PACZero MLX-LM real-parameter one-step update smoke")
    print("This computes a real FD/ZPL sign on a Qwen parameter, applies one sign update, evaluates loss, then restores.")
    ...
    baseline_loss_mean, baseline_losses = mean_loss(model, tokenized)
    plus_losses = losses_for_param_value(model, tokenized, args.param_path, original_copy + args.mu * direction)
    minus_losses = losses_for_param_value(model, tokenized, args.param_path, original_copy - args.mu * direction)
    ...
    candidates = evaluate_update_candidates(model, tokenized, args.param_path, original_copy, direction, release.sign, args.lr)
    # Apply the normal PACZero/ZO descent convention, then restore.  The persisted
    # model is not saved; this is deliberately non-destructive.
    update_value = original_copy - args.lr * float(release.sign) * direction
    update_losses = losses_for_param_value(model, tokenized, args.param_path, update_value)
    update_loss_mean = float(update_losses.mean())
    set_path(model, args.param_path, original_copy)
    restored_loss_mean, restored_losses = mean_loss(model, tokenized)
    ...
    checks = {
        ...
        "update_losses_finite": bool(np.isfinite(update_losses).all()),
        "update_changed_loss": bool(abs(update_loss_mean - baseline_loss_mean) > 0.0),
    ...
    payload = {
        ...
        "parameterization": "real_model_float_parameter_one_step_update",
        ...
        "update_loss_delta": update_loss_mean - baseline_loss_mean,
        "descent_convention_loss_mean": candidates["descent_loss_mean"],
        "opposite_convention_loss_mean": candidates["opposite_loss_mean"],
        "restored_loss_mean": restored_loss_mean,
        ...
        "note": "One-step real-parameter PACZero-ZPL update smoke. It is non-destructive: the original parameter is restored after evaluation.",
    }
    args.json_out.parent.mkdir(parents=True, exist_ok=True)
    args.json_out.write_text(json.dumps(payload, indent=2), encoding="utf-8")
    print("REAL_PARAM_UPDATE_SMOKE_RESULT_JSON=")
